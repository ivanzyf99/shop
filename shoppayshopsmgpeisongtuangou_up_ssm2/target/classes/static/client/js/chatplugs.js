...
function refreshServerChatList(){
    ...
    if(_chattype==2){
        ...
        ajaxCallback("listSqlJ",{sql:"select * from wct_message u where u.msgtype=1 and (u.uid="+uid+" or u.fid="+uid+") and (u.uid="+fid+" or u.fid="+fid+")"},function(data){
            data = data||[];
            if(data.length != _lastmsgcount){
                refreshChatList(data);
                _lastmsgcount = data.length
...
function listMyFriend(){
    ...
    ajaxCallback("listSqlJ",{sql:`select * from wct_user where id in (${fids})`},function(data){
        focuslist = data;
...
function listMyQunzu(){
    ajaxCallback("findJ",{table:"user",id:userinfo.id},function (data) {
        ...
        ajaxCallback("listSqlJ",{sql:`select * from wct_qunzu where id in (${qids})`},function(data){
            focuslist = data;
...
function toSelectUsers(id){
    ...
    if(id){
        ajaxCallback("findJ",{table:"qunzu",id:id},function (data) {
            ...
            ajaxCallback("listSqlJ",{sql:`select * from wct_user where id in (${fids})`},function(ulist){
                focuslist = ulist;
...
function toQunInfo(id){
    ajaxCallback("findJ",{id:id,table:"qunzu"},function (obj) {
        if(obj){
            ...
            ajaxCallback("listSqlJ",{sql:`select * from wct_user where id in (${focusqun.uids})`},function(data){
                focuslist = data;
